...
- **自动剪辑**：一键分析可**无痕剪辑**的位置（能量低谷 + 信号连续 + 网格对齐），弹窗展示剪辑方案（位置/小节/依据/质量分）；最少段长（2/3/5s）与对齐网格可调、方案即时重算，剪切点与每段可试听，摘要显示拼接总时长与保留比例；一键导入下方拼接序列，剪切点实时标记在波形上
...
3. **选段**：在波形上**双击**选中一小节，点「＋ 添加选中区间」加入序列；重复可添加多段；或点「✂ 自动剪辑」一键生成无痕剪辑方案并导入序列
...
**自动剪辑**（`src/autoCut.js`）：积分图 RMS 能量包络 → 局部极小（谷）检测与谷深排序
→ 谷内最小差分定位（信号连续、过零附近）→ 可选吸附最近网格线（小节线优先于拍线）
→ 分段方案（过滤过短段，相邻剪切点间隔与最小段长联动）。剪切点满足「能量低谷 +
波形连续 + 节奏对齐」，导入序列后拼接处无咔嗒声。
